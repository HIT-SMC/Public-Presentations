 ...
 **overfitting**: lots of parameters, induce high-variance/low-bias situation --> unsupervised pretraining (feature learning like autoencoder then finetuning), dropout etc. <br>
 ...
 BN: mini-batch $B =$ {$x_{1,...,m}$},  ${y_i} = BN_{\gamma ,\beta }(x_i) = \gamma {\hat x}_i + \beta$, ${\mu_B = \frac{1}{m} \sum\limits_{i=1}^{m} x_i} $,  ${\simga_B = \frac{1}{m} \sum\limits{i=1}^{m}(x_i - \mu_B)^2} $, ${y_i=\gamma{\hat x}_i + \beta = BN_{\gamma, \beta}(x_i)} $ 
