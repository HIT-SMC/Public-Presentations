 ...
 **overfitting**: lots of parameters, induce high-variance/low-bias situation --> unsupervised pretraining (feature learning like autoencoder then finetuning), dropout etc. <br>
 ...
 BN: mini-batch $B =$ {$x_{1,...,m}$},  ${y_i} = B{N_{\gamma ,\beta }}({{x_i}}) = \gamma {{\hat x}_i} + \beta, <br> \quad
 ${\mu_B = \frac{1}{m} \sum\limits_{i=1}^{m} x_i} $,  <br> \quad
 ${\simga_B = \frac{1}{m} \sum\limits{i=1}^{m}(x_i - \mu_B)^2} $,  <br> \quad
