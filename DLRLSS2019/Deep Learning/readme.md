 ...
 **overfitting**: lots of parameters, induce high-variance/low-bias situation --> unsupervised pretraining (feature learning like autoencoder then finetuning), dropout etc. <br>
 ...
 BN: mini-batch $B =$ {$x_{1,...,m}$},  ${y_i} = BN_{\gamma ,\beta }(x_i) = \gamma {\hat x}_i + \beta$, $ {\mu _{\cal B}} = \frac{1}{m}\sum\limits_{i = 1}^m {{x_i}}, {\sigma _{\cal B}} = \frac{1}{m}\sum\limits_{i = 1}^m {{{\left( {{x_i} - {\mu _{\cal B}}} \right)}^2}},
{{\hat x}_i} = \frac{{x - {\mu _{\cal B}}}}{{\sqrt {\sigma _{\cal B}^2 + \varepsilon } }}$
